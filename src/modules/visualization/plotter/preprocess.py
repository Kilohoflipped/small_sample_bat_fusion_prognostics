...
class PreprocessPlotter:
    ...
    def __init__(self, style_setter: StyleSetter, plot_size_cm: Tuple[float, float], dpi: int):
        ...
        self.dpi = dpi
        self.plot_size_in = (
            self._cm_to_inches(plot_size_cm[0]),
            self._cm_to_inches(plot_size_cm[1]),
        )
    ...
    def _apply_plot_style(
        self, ax: plt.Axes, title: str, xlabel: str, ylabel: str, legend_loc: str = None
    ):
        """
        应用通用绘图风格到 Axes 对象.
        ...
        # 控制网格可见性
        grid_visible = self.style.get("grid_visible", False)
        if grid_visible:
            ax.grid(
                color=self.style.get("grid_color", "#cccccc"),
                alpha=self.style.get("grid_alpha", 0.5),
            )

        # 设置图例风格
        current_legend_loc = (
            legend_loc if legend_loc is not None else self.style.get("legend_loc", "best")
        )
        if ax.get_legend():
            ax.legend(
                prop={"size": self.style.get("legend_fontsize", 19)},
    ...
    def plot_per_battery_comparison(
        self, df_original: pd.DataFrame, df_with_anomaly: pd.DataFrame, output_dir: str
    ):
        """
        ...
        if not all(col in df_original.columns for col in required_cols):
            logger.error(
                f"绘制原始数据与异常点对比图失败: 输入 DataFrame 缺少必要列 {required_cols}"
            )
            return
        if "anomaly" not in df_with_anomaly.columns:
            logger.error(
                "绘制原始数据与异常点对比图失败: 包含异常标记的 DataFrame 缺少 'anomaly' 列"
            )
            return
        ...
        for bid in df_original["battery_id"].unique():
            ...
            with plt.style.context(
                {
                    "font.family": self.style.get("font_family", "sans-serif"),
                ...
                ax.plot(
                    df_bid["cycle_idx"],
                    df_bid["target"],
                    color=self.style.get("line_color_primary", "#3a80f8"),
                    linestyle="-",
                    linewidth=self.style.get("line_linewidth", 1.5),
                    label="原始数据",
                    alpha=0.6,
                )
                ...
                if not df_anomaly_bid.empty:
                    ax.scatter(
                        df_anomaly_bid["cycle_idx"],
                        df_anomaly_bid["target"],
    ...
    def plot_overall_sequence(
        ...
        if len(unique_batteries) > max_batteries:
            logger.warning(
                f"电池数量 ({len(unique_batteries)}) 超过最大绘制数量 ({
        ...
        with plt.style.context(
            ...
            sns.lineplot(
                ...
                linewidth=self.style.get("line_linewidth", 1.5),
                marker="o",
                markersize=self.style.get("scatter_markersize", 50) / 5,
                alpha=0.7,
                ax=ax,
            )
    ...
    def plot_per_battery_imputed(
        self, df_cleaned: pd.DataFrame, df_imputed: pd.DataFrame, output_dir: str
    ):
        """
        ...
        for bid in df_cleaned["battery_id"].unique():
            ...
            merged = pd.merge(
                df_impute_bid[["battery_id", "cycle_idx", "target"]],
            ...
            with plt.style.context(
                ...
                ax.plot(
                    df_impute_bid["cycle_idx"],
                    df_impute_bid["target"],
                    color=self.style.get("background_line_color", "#d3d3d3"),
                    linestyle="-",
                    linewidth=1.0,
                    alpha=0.5,
                    label="_nolegend_",
                ...
                if not df_original_points.empty:
                    ax.scatter(
                        df_original_points["cycle_idx"],
                        df_original_points["target"],
                        color=self.style.get("line_color_primary", "#3a80f8"),
                        marker="o",
                        s=self.style.get("scatter_markersize", 50),
                        label="原始值",
                        alpha=0.8,
                    )
    ...
    def plot_per_battery_denoised(
        ...
        denoised_column: str = "target_denoised",
    ):
        """
        ...
        for bid in df["battery_id"].unique():
            ...
            with plt.style.context(
                ...
                ax.plot(
                    df_bid["cycle_idx"],
                    df_bid[denoised_column],
                    color=self.style.get("line_color_secondary", "#d64f38"),
                    linestyle="-",
                    linewidth=self.style.get("line_linewidth", 1.5),
                    label="去噪后数据",
                    alpha=0.8,
    ...
    def plot_per_battery_decomposed(
        ...
        df: pd.DataFrame,
        output_dir: str,
        target_column: str = "target_denoised",
        ...
        for bid in df["battery_id"].unique():
            ...
            with plt.style.context(
                ...
                if show_modes:
                    ...
                    mode_cols = [
                        col for col in df_bid.columns if col.startswith(f"{target_column}_mode_")
                    ]
                    if mode_cols:
                        # 使用不同的颜色和样式绘制模态
